...
export default function LoginRegister() {
  ...
  async function onRegister(e) {
    ...
    try {
      // Server-Logik: existiert User ohne Passwort → wird „geclaimed“ (Passwort gesetzt)
  ...
  async function onLogin(e) {
    e.preventDefault(); resetMsg();
    try {
      await doLogin({ tenantId, email, password });
    ...
    <Box sx={{ maxWidth: 460, mx: 'auto', mt: 6 }}>
      <Card>
        ...
        <CardContent>
          ...
          <Box component="form" onSubmit={tab === 0 ? onLogin : onRegister} sx={{ display: 'grid', gap: 2 }}>
            <TextField label="Tenant ID" value={tenantId} onChange={e => setTenantId(e.target.value)} required />

            {tab === 1 && (
              <TextField label="Anzeigename" value={displayName} onChange={e => setDisplayName(e.target.value)} required />
            )}
