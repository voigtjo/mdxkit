...
import {
  ...
  FormControlLabel, Button, Alert, Divider, Paper, Table, TableBody, TableCell,
  TableHead, TableRow
} from "@mui/material";

import {
  getFormForPatient, submitForm, getPatient, saveFormData
} from "@/api/userApi";

import { parseFormStructured } from "@/utils/parseFormStructured.jsx";

const UserFormViewStructured = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const parsed = parseFormStructured(res.text, values, handleChange, sigRef);
        setElements(parsed);
      } catch (err) {
...
export default UserFormViewStructured;
